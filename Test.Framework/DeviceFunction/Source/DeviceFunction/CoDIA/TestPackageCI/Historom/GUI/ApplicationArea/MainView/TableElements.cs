...
namespace EH.PCPS.TestAutomation.TestPackageCI.Historom.GUI.ApplicationArea.MainView
{
    using System;
    ...
    public class TableElements
    {
        ...
        public bool FindFirstEvent()
        {
            try
            {
                Controls repo = Controls.Instance;
                string pathToItem = this.mdiClientPath + "/" + repo.Table.eventListTreeContainerInfo.AbsolutePath;
                Container container;
                Host.Local.TryFindSingle(pathToItem, DefaultValues.iTimeoutModules, out container);
                if (container != null)
                {
