...
namespace EH.PCPS.TestAutomation.TestPackageUpdateA2016.Viscosity.Functions.ApplicationArea.MainView.Validation
{
    using System.Diagnostics;
    using System.Reflection;

    using EH.PCPS.TestAutomation.Common.Tools;
    using EH.PCPS.TestAutomation.DeviceFunctionInterfaces.CoDIA.Viscosity.Functions.ApplicationArea.MainView.Validation;
    ...
    public class ModuleOpeningAndClosing : IModuleOpeningAndClosing
    {
        ...
        public bool WaitUntilModuleIsClosed(int timeOutInMilliseconds)
        {
            ...
            {
                Log.Info(
                    LogInfo.Namespace(MethodBase.GetCurrentMethod()),"Module is closed after " + watch.ElapsedMilliseconds + " milliseconds. (Timeout: "
        ...
        public bool WaitUntilModuleIsOpen(int timeOutInMilliseconds)
        {
            ...
            {
                Log.Info(
                    LogInfo.Namespace(MethodBase.GetCurrentMethod()), "Module is opened after " + watch.ElapsedMilliseconds + " milliseconds. (Timeout: " + timeOutInMilliseconds + " milliseconds)");
            }
            else
            {
                Log.Error(
