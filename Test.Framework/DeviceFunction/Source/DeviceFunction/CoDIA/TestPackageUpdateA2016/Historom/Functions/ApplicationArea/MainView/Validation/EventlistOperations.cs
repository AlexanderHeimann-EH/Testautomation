...
namespace EH.PCPS.TestAutomation.TestPackageUpdateA2016.Historom.Functions.ApplicationArea.MainView.Validation
{
    ...
    using EH.PCPS.TestAutomation.TestPackageUpdateA2016.Historom.GUI.StatusArea.Statusbar;
    ...
    public class EventlistOperations : IEventlistOperations
    {
        ...
        ////    Element toolBar = (new StatusBarElements()).StatusBar;
        //    return toolBar != null && Imaging.Contains(toolBar, (new TableElements()).progressbar, Imaging.FindOptions.Default);
        ////}
        ...
        public bool WaitUntilReadFinished(int timeOutInMilliseconds)
        {
            ...
            while (this.IsReading() && this.IsReadFinished() == false)
            {
                ...
                EH.PCPS.TestAutomation.Common.Tools.Log.Error(
                    ...
                    "Eventlist reading did not finish within " + timeOutInMilliseconds + " milliseconds");
                watch.Stop();
                return false;
            }

            watch.Stop();
            EH.PCPS.TestAutomation.Common.Tools.Log.Info(LogInfo.Namespace(MethodBase.GetCurrentMethod()), "Eventlist reading finished after " + watch.ElapsedMilliseconds + " milliseconds. (Timeout: " + timeOutInMilliseconds + " milliseconds)");
            return true;
        }
        ...
        public bool IsReadStarted(int timeOutInMilliseconds)
        {
            ...
            EH.PCPS.TestAutomation.Common.Tools.Log.Error(
                LogInfo.Namespace(MethodBase.GetCurrentMethod()),
                "Reading not started!Check button read/cancel and the progressbar");
            return false;
        }
        ...
        public bool IsReading()
        {
            if (this.IsCancelButtonActive() == false)
            {
                EH.PCPS.TestAutomation.Common.Tools.Log.Info(
                    LogInfo.Namespace(MethodBase.GetCurrentMethod()),
                    "Reading may be finished, button Cancel is inactive");
                return false;
            }
            ...
            {
                EH.PCPS.TestAutomation.Common.Tools.Log.Info(
                    LogInfo.Namespace(MethodBase.GetCurrentMethod()), "Reading may be finished, button Read is active");
                return false;
            }
            ...
            if (this.IsOperationInProgress() == false)
            {
                EH.PCPS.TestAutomation.Common.Tools.Log.Info(
                    LogInfo.Namespace(MethodBase.GetCurrentMethod()),
                    "Reading may be finished, Progressbar is not visible");
                return false;
            }
        ...
        public bool IsCancelButtonActive()
        {
            ...
            if (button == null)
            {
                EH.PCPS.TestAutomation.Common.Tools.Log.Error(LogInfo.Namespace(MethodBase.GetCurrentMethod()), "Cancel button == null!");
                return false;
            }
