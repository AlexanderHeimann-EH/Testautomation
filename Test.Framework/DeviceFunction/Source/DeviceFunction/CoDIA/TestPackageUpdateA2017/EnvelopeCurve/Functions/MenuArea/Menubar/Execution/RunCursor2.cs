//------------------------------------------------------------------------------
// <copyright file="RunCursor2.cs" company="Endress+Hauser Process Solutions AG">
...
namespace EH.PCPS.TestAutomation.TestPackageUpdateA2017.EnvelopeCurve.Functions.MenuArea.Menubar.Execution
{
    ...
    /// </summary>
    public class RunCursor2 : IRunCursor2
    {
        /// <summary>
        ...
        public Element ViaMenu()
        {
            try
            {
                ...
                {
                    // Select entry Cursor2
                    MenuItem menuItem = (new Elements()).SubmenuCursor2;
                    if (menuItem != null && menuItem.Enabled)
                    {
                        Mouse.MoveTo(menuItem, 500);
                        menuItem.Click(DefaultValues.locDefaultLocation);

                        // Select container for Envelope Curve -> Cursor2 menu entries
