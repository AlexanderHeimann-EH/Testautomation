//------------------------------------------------------------------------------
// <copyright file="RunSetCursor.cs" company="Endress+Hauser Process Solutions AG">
...
namespace EH.PCPS.TestAutomation.TestPackageUpdateA2017.EnvelopeCurve.Functions.MenuArea.Menubar.Execution
{
    ...
    /// </summary>
    public class RunSetCursor : IRunSetCursor
    {
        /// <summary>
        ...
        public Element ViaMenu()
        {
            try
            {
                ...
                {
                    // Select entry Set Cursor
                    MenuItem menuItem = (new Elements()).SubmenuSetCursor;
                    if (menuItem != null && menuItem.Enabled)
                    {
                        Mouse.MoveTo(menuItem, 500);
                        menuItem.Click(DefaultValues.locDefaultLocation);

                        // Select container for Envelope Curve -> Set Cursor menu entries
