...
namespace EH.PCPS.TestAutomation.PrototypeHMI20.Parameterization.Functions.ApplicationArea.MainView.Execution
{
    ...
    /// <summary>
    /// Class GetExtendedHeaderParameterUnit.
    /// </summary>
    public class GetExtendedHeaderParameterUnit : IGetExtendedHeaderParameterUnit
    {
        /// <summary>
        /// Gets the value of a header parameter in current display content.
        /// </summary>
                ...
                if (xml.Document != null)
                {
                    ...
                    if (extendedHeaderParams != null && extendedHeaderParams.Any())
                    {
                        foreach (var param in extendedHeaderParams)
                        {
                            ...
                            if (floatValue != null)
                            {
                                var unitString = floatValue.Attribute("unitString");
                                if (unitString != null)
                                {
                                    result = unitString.Value;
                                }
                            }
                        }
                    }
