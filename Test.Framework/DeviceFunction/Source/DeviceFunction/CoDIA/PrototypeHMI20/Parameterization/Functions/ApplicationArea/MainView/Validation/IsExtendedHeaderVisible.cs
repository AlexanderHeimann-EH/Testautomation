namespace EH.PCPS.TestAutomation.PrototypeHMI20.Parameterization.Functions.ApplicationArea.MainView.Validation
{
    ...
    /// </summary>
    public class IsExtendedHeaderVisible : IIsExtendedHeaderVisible
    {
        /// <summary>
        ...
        public bool Run()
        {
            try
            {
                ...
                if (xml.Document != null)
                {
                    ...
                    var deviceHeader = helpPageList?.Element("ExtendedDeviceHeaderDTM");
                    var deviceHeaderAttribute = deviceHeader?.Attribute("isOnScreen");

                    if (deviceHeaderAttribute == null)
                    {
                        Log.Error(LogInfo.Namespace(MethodBase.GetCurrentMethod()), "Extended device header attribute is null.");
                    }
                    else if (deviceHeaderAttribute.Value.Equals("true"))
                    {
                        Log.Info(LogInfo.Namespace(MethodBase.GetCurrentMethod()), "Extended device header is visible.");
                        result = true;
                    }
                    else
                    {
                        Log.Info(LogInfo.Namespace(MethodBase.GetCurrentMethod()), "Extended Device Header is not visible.");
                    }
                }
