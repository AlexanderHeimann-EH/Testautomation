...
namespace EH.PCPS.TestAutomation.FieldCare.V20901.CommonFlows
{
    ...
    public class OpenHostApplication : IOpenHostApplication
    {
        ...
        public bool Run()
        {
            if (HostApplicationLoader.FDT.FieldCare.Functions.ApplicationArea.MainView.Execution.StartFrame.FieldCare())
            {
        ...
        {
            if (HostApplicationLoader.FDT.FieldCare.Functions.ApplicationArea.MainView.Execution.StartFrame.FieldCare(path))
            {
                Log.Info(LogInfo.Namespace(MethodBase.GetCurrentMethod()), "Frame is started successfully");
                return true;
            }
