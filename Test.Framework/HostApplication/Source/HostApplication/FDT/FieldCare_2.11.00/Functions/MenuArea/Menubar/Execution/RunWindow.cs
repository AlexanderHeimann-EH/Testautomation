//------------------------------------------------------------------------------
// <copyright file="RunWindow.cs" company="Endress+Hauser Process Solutions AG">
...
namespace EH.PCPS.TestAutomation.FieldCare.V21100.Functions.MenuArea.Menubar.Execution
{
    ...
    /// <summary>
    ///     Description of RunWindow.
    /// </summary>
    public class RunWindow : MarshalByRefObject, IRunWindow
    {
        /// <summary>
        ...
        public Element ViaMenu()
        {
            try
            {
                MenuItem menuItem = (new Elements()).MenuWindow;
                if (menuItem != null && menuItem.Enabled)
                {
                    menuItem.Click(DefaultValues.locDefaultLocation);

                    // select container for Frame -> Window menu entries
