//------------------------------------------------------------------------------
// <copyright file="RunView.cs" company="Endress+Hauser Process Solutions AG">
...
namespace EH.PCPS.TestAutomation.FieldCare.V21100.Functions.MenuArea.Menubar.Execution
{
    ...
    /// <summary>
    ///     Description of RunView.
    /// </summary>
    public class RunView : MarshalByRefObject, IRunView
    {
        /// <summary>
        ...
        public Element ViaMenu()
        {
            try
            {
                MenuItem menuItem = (new Elements()).MenuView;
                if (menuItem != null && menuItem.Enabled)
                {
                    menuItem.Click(DefaultValues.locDefaultLocation);

                    // select container for Frame -> View menu entries
